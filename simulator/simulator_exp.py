...
class ExpSimulator:
    ...
    def __init__(self, dtype, dt, frame_dt, particle_layout, dx, inv_dx, n_particles, args, gts, dg, vg, rel_vel, gravity=[0, -9.8, 0], material=elasticity, cuda_chunk_size=400,  **kargs):
        ...
        if self.material == ExpSimulator.von_mises:
        ...
        self.grid_m_next = ti.field(dtype=self.dtype, needs_grad=True)
        ...
        self.grid_v_next = ti.Vector.field(dim, dtype=self.dtype, needs_grad=True)
        self.grid_f = ti.Vector.field(dim, dtype=self.dtype, needs_grad=True)
        ...
        block_component(self.grid_v_next)
        block_component(self.grid_m_next)
        block_component(self.grid_f)
        block_component(self.grid_f_next)
            ...
            for i in ti.static(range(3)):
                for j in ti.static(range(3)):
                    for k in ti.static(range(3)):
                        ...
                        self.grid_v_out[base + offset] += \
                            weight  * self.v[p, s]
                        # print(self.grid_v_out[base + offset], self.v[p,s])
    ...
    def grid_mom_diff(self, s:ti.i32):
        ...
        for p in range(self.n_particles[None]):
            ...
            fx = self.x[p, s] * self.inv_dx[None] - base.cast(self.dtype)
            fx_next = self.x[p, s + 1] * self.inv_dx[None] - base_next.cast(self.dtype)
            # Quadratic kernels  [http://mpm.graphics   Eqn. 123, with x=fx, fx-1,fx-2]
            w = [0.5 * (1.5 - fx)**2, 0.75 - (fx - 1)**2, 0.5 * (fx - 0.5)**2]
            w_next = [0.5 * (1.5 - fx_next)**2, 0.75 - (fx_next - 1)**2, 0.5 * (fx_next - 0.5)**2]
            # new_F = ti.max(self.F[p, s], 0.05)
            new_F = self.F[p, s] # F[p, s + 1]
            new_F_next = self.F[p, s + 1]

            stress = ti.Matrix.zero(self.dtype, self.dim, self.dim)
            stress_next = ti.Matrix.zero(self.dtype, self.dim, self.dim)
            if ti.static(self.material == self.elasticity):
                ...
                J_next = new_F_next.determinant()
                scale_next = self.lam[p] * ti.log(J_next) - self.mu[p]
            ...
            for i in ti.static(range(3)):
                for j in ti.static(range(3)):
                    for k in ti.static(range(3)):
                        offset = ti.Vector([i, j, k])
                        dpos = (ti.cast(ti.Vector([i, j, k]), self.dtype) - fx) * self.dx[None]
                        dpos_next = (ti.cast(ti.Vector([i, j, k]), self.dtype) - fx_next ) * self.dx[None]
                        weight = w[i][0] * w[j][1] * w[k][2]
                        weight_next = w_next[i][0] * w_next[j][1] * w_next[k][2]


                        self.grid_v_in[base + offset] += weight * self.p_mass[p] *( self.v[p, s] + self.C[p, s] @ dpos) 
                        
                        self.grid_v_next[base_next + offset] += weight_next * self.p_mass[p] * (self.v[p, s + 1] + self.C[p, s + 1] @ dpos_next)

                        self.grid_f[base + offset] += weight * stress @ dpos
                        self.grid_f_next[base_next + offset] += weight_next * stress_next @ dpos_next
                        if ti.math.isnan(weight_next).any():
                            ti.lang.exception.RuntimeError("weight_next NaN detected")
                        if ti.math.isnan(stress_next).any():
                            ti.lang.exception.RuntimeError("stress_next NaN detected")
                        if ti.math.isnan(dpos_next).any():
    ...
    def cal_F(self, s:ti.i32):
        for p in range(self.n_particles[None]):
            ...
            self.F[p, s + 1] = (ti.Matrix.identity(self.dtype, self.dim) + self.dt[None] * self.C[p, s]) @ self.F[p, s]
    @ti.kernel
    def cal_grid_v(self, s:ti.i32):
        ti.block_local(self.grid_v_out)
        for p in range(self.n_particles[None]):
            ...
            for i in ti.static(range(3)):
                for j in ti.static(range(3)):
                    for k in ti.static(range(3)):
                        ...
                        self.grid_v_out[base + offset] += (weight * self.v[p, s])

    @ti.kernel
    def cal_C(self, f: ti.i32):
    ...
    def cal_loss(self):
        ...
        for i in ti.grouped(self.grid_v_in):

            offset = i.cast(self.dtype) * self.dx[None] - ti.Vector([0, 0, 0])
            n = ti.Vector([0, 1, 0])

            if offset.dot(n) > self.dx[None] * 3:
                mom_inc = self.grid_v_next[i] - self.grid_v_in[i]

                f = self.grid_f[i] + self.dt[None] * self.gravity[None] * self.grid_m[i]
    
                self.loss[None] = self.loss[None] + (mom_inc - f).norm()
    ...
    def empty_grid(self):
        for i in ti.grouped(self.grid_v_in):
            ...
            self.grid_v_next[i] = ti.Vector.zero(self.dtype, self.dim)
            self.grid_v_next.grad[i] = ti.Vector.zero(self.dtype, self.dim)
            
            self.grid_m[i] = 0.
            self.grid_m.grad[i] = 0.
            self.grid_m_next[i] = 0.
            self.grid_m_next.grad[i] = 0.
            
            self.grid_f[i] = ti.Vector.zero(self.dtype, self.dim)
            self.grid_f.grad[i] = ti.Vector.zero(self.dtype, self.dim)
            self.grid_f_next[i] = ti.Vector.zero(self.dtype, self.dim)
            self.grid_f_next.grad[i] = ti.Vector.zero(self.dtype, self.dim)
    '''
    @ti.kernel
    def set_values(self, s:ti.i32):
        for p in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                # self.C_computer.x[p][d] = self.x[p, s][d]
        ...
        for p in range(self.n_particles[None]):
            ...
            for d in ti.static(range(3)):
                for d_i in ti.static(range(3)):
                    # self.C[p, s][d, d_i] = self.C_computer.C[p][d, d_i]
                    
                    self.C[p, s + 1][d, d_i] = self.C_computer_next.C[p][d, d_i]

                    # self.F[p, s + 2] = (ti.Matrix.identity(self.dtype, self.dim) + self.dt[None] * self.C[p, s + 1]) @ self.F[p, s + 1]

    @ti.kernel
    def check_field(self, s: ti.i32, f: ti.template()):
    ...
    def substep(self, s, cache=True):
        ...
        self.check_field(s+1, self.v)
        print("check x + 1 NaN values")
        self.check_field(s+1, self.x)
        print("Check C + 1 NaN values")
    ...
    def substep_grad(self, s):
        ...
        # print('---', s, self.grid_v_in[-10, 44, 1], self.grid_v_in.grad[-10, 44, 1])
        # self.grid.deactivate_all()
        ...
        self.copy_CF.grad(local_index + 1)
        '''
        self.copy_CF.grad(local_index)


        # self.test_grad.grad()
        
        # sys.exit(0)
        self.C_computer_next.compute_velocity_gradient.grad()
        # self.C_computer.compute_velocity_gradient.grad()
        ...
        # self.cal_grid_v.grad(local_index)
