...
from simulator import ExpSimulator
...
def plot_3d_vectors(x, y, save_dir="C_check_plot", gif_name="vectors.gif", duration=0.8):
    n = x.shape[0]
    os.makedirs(save_dir, exist_ok=True)
    filenames = []

    # 全局坐标范围（包含所有中心粒子和邻居）
    all_positions = np.concatenate([x[:, None, :], y], axis=1).reshape(-1, 3)
    xyz_min = np.min(all_positions, axis=0) - 0.5
    xyz_max = np.max(all_positions, axis=0) + 0.5

    # 生成固定的 27 色调色板（均匀分布的 HSV 色相）
    hsv_colors = np.zeros((27, 3))
    hsv_colors[:, 0] = np.linspace(0, 1, 27, endpoint=False)  # hue
    hsv_colors[:, 1] = 0.8  # saturation
    hsv_colors[:, 2] = 1.0  # value
    fixed_colors = hsv_to_rgb(hsv_colors)
    ...
    for i in range(n):
        ...
        center = x[i]
        neighbors = y[i]
        vectors = (neighbors - center) * 10

        # 绘制每个邻居，使用固定的颜色
        for j in range(27):
            ax.quiver(center[0], center[1], center[2],
                      vectors[j, 0], vectors[j, 1], vectors[j, 2],
                      color=fixed_colors[j], linewidth=1.5, arrow_length_ratio=0.2)
        ...
        # 保存每一帧的图片
        fname = os.path.join(save_dir, f"frame_{i:03d}.png")
...
@ti.data_oriented
class Estimator_exp(torch.nn.Module):
    
    velocity_stage = 0
    physical_params_stage = 1

    def __init__(self, phys_args, dtype, gts: list, vels: list, init_vol, 
                 surface_index=None, cuda_chunk_size=100, dynamic_scene=None, 
                 image_scale=1.0, pipeline=None, image_op=None):

        super(Estimator_exp, self).__init__()
        ...
        # rs_basis = torch.linalg.inv(rs[0])
        # for i in self.rs:
        #     self.dg.append(torch.bmm(i , rs_basis))

        # n_n = self.dg[0].shape[0]
        '''
        init_F = torch.eye(3, dtype = torch.float32).repeat(self.gts[0].shape[0], 1, 1)
        for i in range(self.max_f):
            # self.rel_vel.append((gts[i+1] - gts[i])/frame_dt)
            # self.vg.append( torch.bmm(self.dg[i+1], torch.linalg.inv(self.dg[i]) - torch.eye(3).repeat(n_n, 1, 1).cuda()) / self.frame_dt )
            computer = VelocityGradientComputer(num_particles=self.gts[i].shape[0], dx = self.dx)

            computer.set_particles(self.gts[i].detach().cpu().numpy(), self.vels[i].detach().cpu().numpy())
            computer.find_neighbors()

            computer.compute_velocity_gradient()

            self.vg.append(torch.from_numpy(computer.C.to_numpy()))
            if i == 0:
                self.dg.append(init_F)
            else:
                init_F = torch.bmm((torch.eye(3, dtype = torch.float32).repeat(self.gts[i - 1].shape[0], 1, 1) + self.frame_dt * self.vg[i - 1]) , init_F)
                self.dg.append(init_F)
        '''

        # self.volumes = volumes

        # self.rel_vel = torch.stack(self.rel_vel, dim = 0)
        self.vels = torch.stack(self.vels, dim =0).detach()
        self.vels = self.vels.to(init_vol.device).requires_grad_(True)
        # self.vg = torch.stack(self.vg, dim = 0)
        self.gts = torch.stack(self.gts, dim = 0).detach()
        self.gts = self.gts.to(init_vol.device).requires_grad_(True)
        # self.dg = torch.stack(self.dg, dim = 0)
        ...
        self.m_f = ti.field(ti.i32, shape=())
        self.m_f[None] = self.max_f
        ...
        self.gt = ti.Vector.field(n=3, dtype=self.dtype, shape=(max_f+1, surface_count), needs_grad=True)
        ...
        self.num_particles_surface = ti.field(ti.i32, shape=(max_f+1)) # gt surface particles count per frame
        self.num_particles[None] = particles_count
        ...
        # self.nu_bound = getattr(phys_args, "nu_bound", [-0.9, 0.3])
        ...
        self.simulator = ExpSimulator(dtype=self.dtype, dt=dt, frame_dt=frame_dt, n_particles=self.num_particles,
                                      material=phys_args.material, dx=self.dx, inv_dx=self.inv_dx, 
                                      particle_layout=self.particle, args=phys_args, gravity=gravity, 
                                      cuda_chunk_size=cuda_chunk_size,  gts = self.gts, dg = self.dg, vg = self.vg, rel_vel = self.vels)
    ...
    def initialize(self):
        ...
        self.image_loss = 0.0
        self.loss[None] = 0.0
        self.simulator.loss[None] = 0.0
        cnt = self.init_vol.shape[0]
        # velocities = self.init_vel.repeat(cnt).reshape(cnt, -1)
        velocities = self.vels[0] # The first frame velocity
        ...
        # self.init_rhos = self.global_rho.expand(self.num_particles[None]).clone()
        particle_rho = self.init_rhos

        self.clear_grads()
        if self.material == ExpSimulator.elasticity:
        ...
        elif self.material == ExpSimulator.von_mises:
            if getattr(self.args, 'init_E', None) and getattr(self.args, 'init_nu', None):
                ...
                self.init_lam = lam.repeat(cnt)
        elif self.material == ExpSimulator.viscous_fluid:
            self.init_mu = (10 ** self.global_mu).repeat(cnt)
            lam = 10 ** self.global_kappa - 2./3. * 10 ** self.global_mu
            self.init_lam = lam.repeat(cnt)
        elif self.material == ExpSimulator.drucker_prager:
            nu = self.get_nu()
            self.init_mu =  ((10**self.E) / (2. * (1. + nu))).repeat(cnt)
            self.init_lam = ((10**self.E) * nu / ((1. + nu) * (1. - 2. * nu))).repeat(cnt)
        ...
        self.init_pos = self.init_vol.clone().detach().requires_grad_(False)
        self.init_velocities = velocities
        self.simulator.cached_states.clear()

        # self.from_torch(particles.data.cpu().numpy(), velocities.data.cpu().numpy(), particle_rho.data.cpu().numpy(), self.init_mu.data.cpu().numpy(), self.init_lam.data.cpu().numpy(), self.dg[0].data.cpu().numpy(), self.vg[0].data.cpu())
        # self.from_torch(self.gts.data.cpu().numpy(), self.vels.data.cpu().numpy(), particle_rho.data.cpu().numpy(), self.init_mu.data.cpu().numpy(), self.init_lam.data.cpu().numpy(), self.dg.data.cpu().numpy(), self.vg.data.cpu())
        self.from_torch(self.gts.data.cpu().numpy(), self.vels.data.cpu().numpy(), particle_rho.data.cpu().numpy(), self.init_mu.data.cpu().numpy(), self.init_lam.data.cpu().numpy())
    ...
    def from_torch(self, particles: ti.types.ndarray(), 
        ...
        for p in range(self.num_particles[None]):
            ...
            '''
                for d in ti.static(range(3)):
                    for d_i in ti.static(range(3)):
                        self.simulator.F[p, s][d, d_i] = dg[s, p, d, d_i]
                        
                        self.simulator.C[p, s][d, d_i] = vg[s, p, d , d_i]
            '''
    '''
    def update_volume(self, t):
        self.update_volume_from_torch(t,  self.volumes.data.cpu().numpy())
    @ti.kernel
    def update_volume_from_torch(self, t:ti.i32, volumes: ti.types.ndarray()):
        for p in range(self.num_particles[None]):
            self.simulator.p_vol[p] = volumes[t, p]
    '''
    def succeed(self):
    ...
    def forward(self, f, img_backward=True):
        ...
        self.simulator.advance(f)
        
        # print(">>>:", f)
        # print('V', self.simulator.v[10000, f], self.simulator.v[0, f], self.simulator.v[20000, f])
        '''
        print('C:')
        print(self.simulator.C[10000, f])
        print(self.simulator.C[10000, f + 1])
        print('F:')
        print(self.simulator.F[10000, f])
        print(self.simulator.F[10000, f + 1])
        '''
    ...
    @ti.kernel
    def get_input_grad(self, rho_grad: ti.types.ndarray(),
                             mu_grad: ti.types.ndarray(), lam_grad: ti.types.ndarray(), velocity_grad: ti.types.ndarray(), position_grad: ti.types.ndarray(), s:ti.i32 ):
        for p in range(self.num_particles[None]):
            ...
            for d in ti.static(range(3)):
                velocity_grad[s, p, d] = self.simulator.v.grad[p, s][d]
                position_grad[s, p, d] = self.simulator.x.grad[p, s][d]
            # print("rho, mu and lam:",s, ":", rho_grad[p], mu_grad[p], lam_grad[p])
            # print("vels and pos:", s, ":", velocity_grad[s, p, 0], velocity_grad[s, p, 1], velocity_grad[s, p, 2],  position_grad[s , p, 0], position_grad[s , p, 1], position_grad[s , p, 2])
      
    def backward(self, f):
        local_index = (f * self.simulator.n_substeps[None]) % self.simulator.cuda_chunk_size


        self.simulator.advance_grad(f)
        self.compute_particle_mass.grad()
        dtype=np.float32

        velocity_grad = np.zeros([self.max_f + 1, self.num_particles[None], 3], dtype=dtype)
        position_grad = np.zeros([self.max_f + 1, self.num_particles[None], 3], dtype=dtype)
        ...
        self.get_input_grad(rho_grad, mu_grad, lam_grad, velocity_grad, position_grad, local_index)
        ...
        return torch.from_numpy(mu_grad).to(self.device), torch.from_numpy(lam_grad).to(self.device), \
                ...
                torch.from_numpy(rho_grad).to(self.device), torch.from_numpy(velocity_grad).to(self.device), torch.from_numpy(position_grad).to(self.device)
